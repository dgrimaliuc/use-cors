...
async function getHandler(req, res) {
  if (!hasRequiredHeaders(req.headers)) {
    res.writeHead(401, 'Required header is not set');
    res.json(errorMessage('Content-Type not set or request is unauthorized'));
    return;
  }
  ...
  if (!location) {
    if (/https?:\/\/.+/i.test(url)) {
      ...
      res.json(errorMessage(`The URL is invalid: '${url}'`));
      return;
    }
    return;
  }
  ...
  if (!isValidHostName(location.hostname)) {
    ...
    res.json(errorMessage('Invalid host: ' + location.hostname));
    return;
  }
